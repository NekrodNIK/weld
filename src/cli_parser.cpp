...
LinkerFlags* parse_cli_options(int argc, char* argv[]) {
    ...
    options.add_options()
        ...
        ("m,target", "Architecture", cxxopts::value<std::string>()->default_value("elf_x86_64"))
    ;
    ...
    if (result["target"].as<std::string>() == "elf_x86_64") {
        lf->arch = Arch::elf_x86_64;
        lf->arch_name = result["target"].as<std::string>();
    } else {
        throw InvalidArchitecture(result["target"].as<std::string>());
    }
