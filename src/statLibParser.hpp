    ...
    static Archive read(const std::string& filename) {
        ...
        while (reader.peek() != EOF) {
            ...
            if (header.ar_fmag[0] != 0x60 || header.ar_fmag[1] != 0x0A) {
                throw std::ios::failure("Invalid archive. Failed on index " + std::to_string(reader.peek()));
            }
            ...
            if (reader.gcount() != size) {
                throw std::ios::failure("Invalid archive. File has diiferent size than in header");
            }
...
class ArWriter {
    ...
    static void write(const std::string& archname, const std::vector<std::string>& filenames) {
        std::ofstream writer(archname, std::ios::binary); 
        if (!writer) {
            throw std::ios::failure("Couldn't open archive file for writing: " + archname);
        }
        ...
        for (const auto& path : filenames) {
            ...
            memcpy(fileHeader.ar_date, tmp, std::min<size_t>(len, sizeof(fileHeader.ar_date)));

            len = snprintf(tmp, sizeof(tmp), "%6d", getuid());
            memcpy(fileHeader.ar_uid, tmp, std::min<size_t>(len, sizeof(fileHeader.ar_uid)));

            len = snprintf(tmp, sizeof(tmp), "%6d", getgid());
            memcpy(fileHeader.ar_gid, tmp, std::min<size_t>(len, sizeof(fileHeader.ar_gid)));

            len = snprintf(tmp, sizeof(tmp), "%8o", 0644);
            memcpy(fileHeader.ar_mode, tmp, std::min<size_t>(len, sizeof(fileHeader.ar_mode)));

            len = snprintf(tmp, sizeof(tmp), "%10zu", data.size());
            memcpy(fileHeader.ar_size, tmp, std::min<size_t>(len, sizeof(fileHeader.ar_size)));
