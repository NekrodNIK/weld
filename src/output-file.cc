...
void OutputFile<E>::write(Context<E>& ctx, const std::filesystem::path& path) {
  ...
  auto add_section_header = [&](const std::string& name, u32 type, u32 flags,
                                size_t size, size_t addr, u32 link = 0,
                                u32 info = 0, size_t entsize = 0) {
    elf::Shdr<E> shdr{};
  ...
  if (bss_size > 0)
    bss_idx = add_section_header(".bss", elf::SHT_NOBITS,
                                 elf::SHF_ALLOC | elf::SHF_WRITE, bss_size, 0);

  size_t sym_idx = add_section_header(".symtab", elf::SHT_SYMTAB, 0,
                                      symtab.size() * sizeof(elf::Sym<E>), 0, 0,
                                      1, sizeof(elf::Sym<E>));
  size_t str_idx =
      add_section_header(".strtab", elf::SHT_STRTAB, 0, strtab.size(), 0);
  size_t shstr_idx =
      add_section_header(".shstrtab", elf::SHT_STRTAB, 0, shstrtab.size(), 0);
  ...
  if (!is_rel) {
    ...
    if (!text_bytes.empty()) {
      ...
      ph.p_flags = elf::PF_R | elf::PF_X;
      ph.p_align = PAGE;
      ph.p_offset = text_offset;
      ph.p_vaddr = text_vaddr;
      ph.p_paddr = text_vaddr;
      ph.p_filesz = text_bytes.size();
      ph.p_memsz = text_bytes.size();
      phdrs.push_back(ph);
    }
    if (!rodata_bytes.empty()) {
      elf::Phdr<E> ph{};
      ...
      ph.p_flags = elf::PF_R;
      ph.p_align = PAGE;
      ph.p_offset = rodata_offset;
      ph.p_vaddr = rodata_vaddr;
      ph.p_paddr = rodata_vaddr;
      ph.p_filesz = rodata_bytes.size();
      ph.p_memsz = rodata_bytes.size();
      phdrs.push_back(ph);
    }
    if (!data_bytes.empty() || bss_size > 0) {
      elf::Phdr<E> ph{};
      ph.p_type = elf::PT_LOAD;
      ph.p_flags = elf::PF_R | elf::PF_W;
      ph.p_align = PAGE;
      ph.p_offset = data_offset;
      ph.p_vaddr = data_vaddr;
      ph.p_paddr = data_vaddr;
      ph.p_filesz = data_bytes.size();
      ph.p_memsz = data_bytes.size() + bss_size;
      phdrs.push_back(ph);
    }

    if (text_idx)
      shdr_tab[text_idx].sh_addr = text_vaddr;
    if (rodata_idx)
      shdr_tab[rodata_idx].sh_addr = rodata_vaddr;
    if (data_idx)
      shdr_tab[data_idx].sh_addr = data_vaddr;
    if (bss_idx)
      shdr_tab[bss_idx].sh_addr = data_vaddr + data_bytes.size();
    if (text_idx)
      shdr_tab[text_idx].sh_offset = text_offset;
    if (rodata_idx)
      shdr_tab[rodata_idx].sh_offset = rodata_offset;
    if (data_idx)
      shdr_tab[data_idx].sh_offset = data_offset;
    if (bss_idx)
      shdr_tab[bss_idx].sh_offset = 0;
  } else {
  ...
  size_t strtab_offset =
      align_addr(symtab_offset + symtab.size() * sizeof(elf::Sym<E>), PAGE);
  size_t shdr_offset = align_addr(strtab_offset + strtab.size(), PAGE);
  size_t shstrtab_offset =
      align_addr(shdr_offset + shdr_tab.size() * sizeof(elf::Shdr<E>), PAGE);
  ...
  copy_tasks.push_back([&]() {
    ...
    if (!phdrs.empty())
      std::memcpy(static_cast<char*>(map) + sizeof(ehdr), phdrs.data(),
                  phdrs.size() * sizeof(elf::Phdr<E>));
  });

  if (!text_bytes.empty())
    copy_tasks.push_back([&]() {
      std::memcpy(static_cast<char*>(map) + text_offset, text_bytes.data(),
                  text_bytes.size());
    });
  if (!rodata_bytes.empty())
    copy_tasks.push_back([&]() {
      std::memcpy(static_cast<char*>(map) + rodata_offset, rodata_bytes.data(),
                  rodata_bytes.size());
    });
  if (!data_bytes.empty())
    copy_tasks.push_back([&]() {
      std::memcpy(static_cast<char*>(map) + data_offset, data_bytes.data(),
                  data_bytes.size());
    });

  copy_tasks.push_back([&]() {
    std::memcpy(static_cast<char*>(map) + symtab_offset, symtab.data(),
                symtab.size() * sizeof(elf::Sym<E>));
  });
  copy_tasks.push_back([&]() {
    std::memcpy(static_cast<char*>(map) + strtab_offset, strtab.data(),
                strtab.size());
  });
  copy_tasks.push_back([&]() {
    std::memcpy(static_cast<char*>(map) + shdr_offset, shdr_tab.data(),
                shdr_tab.size() * sizeof(elf::Shdr<E>));
  });
  copy_tasks.push_back([&]() {
    std::memcpy(static_cast<char*>(map) + shstrtab_offset, shstrtab.data(),
                shstrtab.size());
  });
